...
const HistoryDetail = (props) => {
  ...
  useEffect(() => {
    if (visible) {
      getHistoryDetail(props.router.query.order_id);
  ...
  const getHistoryDetail = (id) => {
    // console.log({ shop_id: shop.id });
    API.getHistoryDetail({ order_id: id })
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        ...
        Swal.fire({
          ...
          window.location.href = "/shop/" + shop.id + "/history/";
        });
        //   window.location.href = "/";
      });
  };
    ...
    <ThemeProvider theme={mdTheme}>
      {visible ? (
        <Box sx={{ display: "flex" }}>
          <CssBaseline />
          <MenuShop
            header={"รายละเอียดประวัติการสั่งซื้อ"}
            shop_id={shop?.id}
          ></MenuShop>
          <Box
            component="main"
...
export default connect((state) => state)(withRouter(HistoryDetail));
